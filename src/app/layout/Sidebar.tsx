...
import { useCanAccessLegal } from '@/hooks/useCanAccessLegal';
...
export function Sidebar() {
  ...
  const { canAccess: canAccessLegal } = useCanAccessLegal();
  const items = NAV_ITEMS.filter(
    (item) =>
      (!item.adminOnly || canSeeAdminNav(role))
      && (!item.module || hasModule(item.module))
      && (!item.legalOnly || canAccessLegal),
  );
        ...
        {items.map((item) => {
          ...
          return (
            <NavLink
              ...
              className={({ isActive }) =>
                cn(
                  'flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium transition-[color,background-color,border-color,box-shadow,opacity,transform] duration-150',
