...
export function DealDrawer({ deal, stages, pipelines, isAdmin, onClose, onStageChange, onChanged }: DealDrawerProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end" role="dialog" aria-label="Detalhe do lead">
      ...
      <aside
        className="relative z-10 flex h-full w-full max-w-full flex-col overflow-y-auto border-l border-[var(--color-border-card)] bg-[var(--color-bg-elevated)] shadow-[0_0_60px_rgb(var(--accent-rgb)/0.12)] transition-transform duration-[400ms] ease-[cubic-bezier(0.4,0,0.2,1)] sm:w-[460px]"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="sticky top-0 z-10 flex items-start justify-between gap-3 border-b border-[rgb(var(--accent-rgb)/0.12)] bg-[var(--color-bg-elevated)]/95 px-5 py-4 backdrop-blur">
        ...
        {loading && !contact ? (
          ...
          <div className="space-y-6 p-5">
            ...
            <section>
              {deal.archived_at ? (
                <div className="flex items-center justify-between gap-2 rounded-lg border border-[rgb(var(--accent-rgb)/0.2)] bg-white/[0.02] p-3 text-xs text-[var(--color-text-secondary)]">
                  <span>Arquivado em {fmtDate(deal.archived_at)}</span>
                  <button
                    onClick={restore}
                    className="inline-flex items-center gap-1.5 rounded-md border border-[rgb(var(--accent-rgb)/0.25)] px-2.5 py-1 text-xs font-semibold text-[var(--accent-secondary)] transition hover:border-[var(--accent-primary)]"
                  >
                ...
                <button
                  onClick={archive}
                  className="inline-flex items-center gap-1.5 rounded-md border border-[rgb(var(--accent-rgb)/0.2)] px-2.5 py-1.5 text-xs font-medium text-[var(--color-text-secondary)] transition hover:border-[var(--accent-primary)] hover:text-[var(--color-text-primary)]"
                >
            ...
            <Link to={`/inbox?contact=${deal.contact_id}`} className="flex items-center justify-center gap-2 rounded-lg border border-[rgb(var(--accent-rgb)/0.2)] py-2.5 text-sm font-medium text-[var(--accent-secondary)] transition hover:border-[var(--accent-primary)] hover:bg-white/5">
            ...
            <section className="space-y-3">
              ...
              <div className="flex gap-2">
                ...
                <button onClick={submitNote} disabled={savingNote || !note.trim()} className="rounded-lg bg-gradient-to-br from-[#1E3A8A] to-[var(--accent-primary)] px-4 py-2 text-sm font-semibold text-white transition hover:opacity-90 disabled:opacity-60">
                  {savingNote ? '…' : 'Anotar'}
                </button>
              </div>
