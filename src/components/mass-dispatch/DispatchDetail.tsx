      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
          {[
            ...
            <div key={s.label} className="rounded-lg border border-[rgb(var(--accent-rgb)/0.12)] bg-white/[0.02] p-3 text-center">
        ...
        {slices.length > 0 && (
          <div className="h-56 rounded-lg border border-[rgb(var(--accent-rgb)/0.1)] bg-white/[0.02] p-2">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                ...
                <Legend />
                <Tooltip contentStyle={{ background: 'var(--color-bg-elevated)', border: '1px solid rgb(var(--accent-rgb)/0.2)', borderRadius: 8 }} />
              </PieChart>
            </ResponsiveContainer>
          </div>
        )}
        ...
        <div className="space-y-2">
          ...
          {loading ? (
            ...
            <div className="overflow-x-auto rounded-lg border border-[rgb(var(--accent-rgb)/0.12)]">
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-[rgb(var(--accent-rgb)/0.12)] text-left text-xs uppercase tracking-wide text-[var(--color-text-secondary)]">
                ...
                <tbody>
                  {contacts.map((c) => (
                    <tr key={c.id} className="border-b border-[rgb(var(--accent-rgb)/0.06)] last:border-0">
