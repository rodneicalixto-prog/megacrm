-- ============================================================================
-- Tracking Inteligente de Origem de Leads · Fase 4
-- ----------------------------------------------------------------------------
-- RPC de reconciliação: mensagem inbound → atribuição no lead (deal). Aplica a
-- hierarquia da spec (seção 2), atômica e testável via SQL:
--   1. CTWA (referral.ctwa_clid presente)     → attribution_method = ctwa
--   2. Código de rastreio [XXXX] no texto     → attribution_method = codigo_rastreio
--   3. Nenhum sinal                           → whatsapp_direto / manual
-- Find-or-create do deal do contato; a trigger _derive_deal_traffic deriva
-- traffic_type/origin_channel e impede rebaixar um attribution_method mais forte.
-- ============================================================================
...
BEGIN
  ...
  IF v_ctwa IS NOT NULL THEN
    v_method  := 'ctwa';
    v_origin  := 'meta_ads';   -- CTWA é sempre Meta; granularidade fb/ig não vem no referral
    v_traffic := 'pago';
  ...
  ELSE
    ...
    IF v_session.id IS NOT NULL THEN
      ...
      -- origin/traffic saem da trigger a partir das UTMs copiadas.
    ELSE
      -- 3. Sem sinal → WhatsApp direto -----------------------------------
      v_method  := 'manual';
  ...
  ELSE
    -- UPDATE dispara a trigger (deriva + no-downgrade do attribution_method).
  ...
  -- Marca a sessão como reconciliada.
...
-- Chamável apenas por service role (Edge Function). Revoga de anon/authenticated.
